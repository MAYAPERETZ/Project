/*
 * RISCVTokenMarker.java - MIPS Assembly token marker
...
public class RISCVTokenMarker extends TokenMarker {
   ...
   public static String[] getRISCVTokenLabels() {
      if (tokenLabels == null) {
            ...
            tokenLabels[Token.KEYWORD1] = "RISCV instruction";
   ...
   public static String[] getRICVTokenExamples() {
   ...
   */
   public ArrayList getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
         ...
         if (token != null && token.id == Token.KEYWORD2) {
            if (moreThanOneKeyword)
               return (keywordType == Token.KEYWORD1) ? getTextFromInstructionMatch(keywordTokenText, true)
                                                      : getTextFromDirectiveMatch(keywordTokenText,true);
            else return getTextFromDirectiveMatch(tokenText, false);
         }
   ...
   /**
   *  Get KeywordMap containing all RISCV key words.  This includes all instruction mnemonics,
   ...
   */
   public static KeywordMap getKeywords() {
      if(cKeywords == null) {
         ...
         ArrayList<mars.riscv.hardware.Register.FPRegister> coprocessor1RegisterFile = FPRegisters.getRegisters();
         for (Register.FPRegister fpRegister : coprocessor1RegisterFile)
            cKeywords.add(fpRegister.getName(), Token.KEYWORD3);
